...
<AppModal
	open={modalOpen}
	title="Charges récurrentes — édition"
	titleId="modal-charges-title"
	onClose={() => (modalOpen = false)}
>
	<div class="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3">
		<Input label="Taxe foncière (€/an)" id="property-tax-emb" type="number" min={0} bind:value={project.charges.propertyTax} />
		<Input label="Charges de copropriété (€/an)" id="co-ownership-fees-emb" type="number" min={0} bind:value={project.charges.coOwnershipFees} />
		<Input label="Gestion locative (€/an)" id="management-fees-emb" type="number" min={0} bind:value={project.charges.managementFees} />
		<Input label="Assurances (€/an)" id="insurance-emb" type="number" min={0} bind:value={project.charges.insurance} />
		<Input label="Factures (électricité, eau…) (€/an)" id="utilities-emb" type="number" min={0} bind:value={project.charges.utilities} />
		<Input label="Honoraires comptables (€/an)" id="accounting-fees-emb" type="number" min={0} bind:value={project.charges.accountingFees} />
		<Input label="Provision pour entretien (€/an)" id="maintenance-provision-emb" type="number" min={0} bind:value={project.charges.maintenanceProvision} />
	</div>
	<div class="mt-3 space-y-1 border-t border-fa-outline-variant/20 pt-3">
		...
		<div class="flex items-center justify-between text-sm">
			...
			<span class="font-semibold text-fa-primary-container">{totalRecoverableCharges.toLocaleString('fr-FR')} €</span>
		</div>
		<div class="mt-2 flex items-center justify-between border-t border-fa-outline-variant/20 pt-2">
			...
			<span class="text-lg font-semibold text-fa-primary-container">{chargesUsedForCalculation.toLocaleString('fr-FR')} €</span>
		</div>
	</div>
</AppModal>
