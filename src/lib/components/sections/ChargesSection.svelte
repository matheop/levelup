<script lang="ts">
	...
	let { charges, embedded = false } = $props<{
		charges: Charges;
		embedded?: boolean;
	}>();
...
{#if embedded}
	<div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
		<Input label="Taxe foncière (€/an)" id="property-tax-emb" type="number" min={0} bind:value={charges.propertyTax} />
		<Input label="Charges de copropriété (€/an)" id="co-ownership-fees-emb" type="number" min={0} bind:value={charges.coOwnershipFees} />
		<Input label="Gestion locative (€/an)" id="management-fees-emb" type="number" min={0} bind:value={charges.managementFees} />
		<Input label="Assurances (€/an)" id="insurance-emb" type="number" min={0} bind:value={charges.insurance} />
		<Input label="Factures (électricité, eau…) (€/an)" id="utilities-emb" type="number" min={0} bind:value={charges.utilities} />
		<Input label="Honoraires comptables (€/an)" id="accounting-fees-emb" type="number" min={0} bind:value={charges.accountingFees} />
		<Input label="Provision pour entretien (€/an)" id="maintenance-provision-emb" type="number" min={0} bind:value={charges.maintenanceProvision} />
	</div>
	<div class="mt-3 pt-3 border-t border-slate-200 space-y-1">
		<div class="flex justify-between items-center">
			...
			<span class="font-semibold text-slate-900">{totalAnnualCharges.toLocaleString('fr-FR')} €</span>
		</div>
		<div class="flex justify-between items-center text-sm">
			<span class="text-slate-600">dont récupérables</span>
			<span class="font-semibold text-slate-900">{totalRecoverableCharges.toLocaleString('fr-FR')} €</span>
		</div>
		<div class="flex justify-between items-center border-t border-slate-200 pt-2 mt-2">
			...
			<span class="text-lg font-semibold">{chargesUsedForCalculation.toLocaleString('fr-FR')} €</span>
		</div>
	</div>
{:else}
